...
The queue-definition `status` controls whether D1 configuration may be loaded (`ACTIVE` or `ARCHIVED`); live `OPEN`/`PAUSED`/`CLOSED` status belongs only to the DO session. The validated D1 reader resolves by queue ID or public slug only when the queue, shop, and owner merchant are active, orders active services by `sort_order` then ID, and injects the fixed V1 buffer. It does not authorize callers; merchant routes must authorize shop membership before using queue-ID lookup. A configuration with no active services fails closed.
